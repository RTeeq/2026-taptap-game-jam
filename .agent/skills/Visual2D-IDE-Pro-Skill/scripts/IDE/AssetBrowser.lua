...
AssetBrowser._categories = {
  image = { name = "图片", icon = "🖼", extensions = {".png", ".jpg", ".jpeg", ".bmp", ".gif"} },
  audio = { name = "音频", icon = "🎵", extensions = {".ogg", ".wav", ".mp3"} },
  script = { name = "脚本", icon = "📜", extensions = {".lua"} },
  font = { name = "字体", icon = "🔤", extensions = {".ttf", ".otf"} },
  data = { name = "数据", icon = "📊", extensions = {".json", ".xml", ".csv"} },
  other = { name = "其他", icon = "📦", extensions = {} },
}
...
function AssetBrowser:ScanAssets()
  ...
  for _, dir in ipairs(dirs) do
    if fileSystem:DirExists(dir) then
      local files = fileSystem:ScanDir(dir)
      for _, fileInfo in ipairs(files) do
        if not fileInfo.isDirectory then
          self:_addAsset(dir .. "/" .. fileInfo.name, fileInfo.size)
        end
      end
    end
  end
